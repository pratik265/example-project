...
const DoctorDetail = () => {
  ...
  if (userLoading || loading) return <div className="loading">Loading doctor information...</div>;
    ...
    <div className="detail-page">
      <div className="container">
          ...
          <div className="cards-grid">
            {doctorClinics.map(clinic => (
              <div key={clinic.id} className="card">
                ...
                <Link to={createUserLink(`/clinics/${clinic.id}`)} className="btn btn-secondary">
                  View Clinic Details
                </Link>
              </div>
            ))}
        ...
        <div style={{marginTop: '2rem', background: 'white', padding: '2rem', borderRadius: '12px', boxShadow: '0 4px 15px rgba(0, 0, 0, 0.1)'}}>
          <h2 style={{color: '#2c3e50', marginBottom: '1rem'}}>About Dr. {doctor.name.split(' ').pop()}</h2>
        ...
        <div style={{textAlign: 'center', marginTop: '2rem', padding: '2rem', background: '#f8f9fa', borderRadius: '12px'}}>
          ...
          <div style={{display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap'}}>
            <Link to={createUserLink('/contact')} className="btn btn-primary">Book Appointment</Link>
            <Link to={createUserLink('/doctors')} className="btn btn-secondary">View Other Doctors</Link>
          </div>
        </div>
      </div>
    </div>
